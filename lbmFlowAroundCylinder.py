...
for time in range(maxIter):
    ...
    if (time%100==0): # Visualization
        plt.clf(); plt.imshow(sqrt(u[0]**2+u[1]**2).transpose(),cmap=cm.viridis)
        ax = plt.axes()
        ax.xaxis.set_visible(False)
        ax.yaxis.set_visible(False)
        plt.savefig("vel."+str(time/100).zfill(4)+".png", bbox_inches='tight')

        if (time>100000):
            data = {'velocity' : u}
            savemat('vel_snapshot.'+str(time/100).zfill(4), mdict=data)
